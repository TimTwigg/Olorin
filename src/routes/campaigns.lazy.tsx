        ...
        api.getCampaign(name).then((res) => {
            ...
            if (Types.length === 0) {
                api.getTypes().then((res) => {
                    console.log(res);
                    if (res && res.Types) SetTypes(res.Types);
                });
            }
        });
    };
        ...
        >
            <div className="playScreen container">
                ...
                <section id="campaignPlayerArea" className="flex">
                    {activeCampaign.Players.map((player, index) => {
                        let foot = <div className="right"><ButtonGroup>
                ...
                footer={<div>
                    ...
                    <Button label="Done" onClick={() => {
                        if (LocalPlayerState) {
                            ...
                            api.editPlayer(p).then((res) => {
                                if (res) {
                                    SetActiveCampaign(activeCampaign.withUpdatedPlayer(res));
            ...
            >
                <div className = "inputContainer">
                    ...
                    <AutoComplete id="player-type" className="margin-bottom" value={LocalStringState3} suggestions={TypeSelectionOptions} completeMethod={(e) => {SetTypeSelectionOptions(Types.filter(c => c.toLowerCase().startsWith(e.query.toLowerCase())))}} onChange={(e: any) => SetLocalStringState3(e.value)} placeholder="Select Type" invalid={LocalStringState3 === ""} dropdown forceSelection />
                    <br />
