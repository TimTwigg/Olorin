            ...
            {actions.map((a, k) => (
                <section key={k}>
                    ...
                    {a.ToHitModifier && (
                        <>
                            {a.ToHitModifier > 0 ? "+" : ""}
                            {a.ToHitModifier} to hit, {a.AttackType?.startsWith("Melee") ? "reach" : "range"} {a.Reach} ft., {a.Targets} creature{a.Targets && a.Targets > 1 ? "s" : ""}.{" "}
                        </>
                    )}
                    {a.Damage && (
                        <>
                            <i>Hit:</i> {a.Damage.map((d) => d.Amount + " " + d.Type + " damage").join(", ")}.{" "}
                        </>
                    )}
    ...
    return (
        <div className="statblock displayCard" style={dynamicStyles}>
            ...
            <Card>
                ...
                <strong>Speed:</strong>{" "}
                {Object.entries(statBlock.Stats.Speed).map(([key, value]) => {
                    return value !== 0 ? (
                        <span key={key}>
                            {key}: {value} ft.{" "}
                        </span>
            ...
            <Card>
                {statBlock.Details.Skills.length > 0 && (
                    <>
                        <strong>Skills:</strong>{" "}
                        {statBlock.Details.Skills.map(({ Name, Level, Override }) => {
                            const num = Override !== 0 ? Override : modifierOf(statBlock.Stats.Abilities.dGet(Name, 10)) + Level * statBlock.ProficiencyBonus;
                            return (
                                <span key={Name}>
                                    {Name} {num > 0 ? "+" : ""}
                                    {num}{" "}
                ...
                {statBlock.Details.SavingThrows.length > 0 && (
                    <>
                        <strong>Saving Throws:</strong>{" "}
                        {statBlock.Details.SavingThrows.map(({ Name, Level, Override }) => {
                            const num = Override !== 0 ? Override : modifierOf(statBlock.Stats.Abilities.dGet(Name, 10)) + Level * statBlock.ProficiencyBonus;
                            return (
                                <span key={Name}>
                                    {Name} {num > 0 ? "+" : ""}
                ...
                {statBlock.DamageModifiers.Immunities.length > 0 && (
                    <>
                        <strong>Damage Immunities:</strong> {statBlock.DamageModifiers.Immunities.join(", ")}
                        <br />
                    </>
                )}
                {statBlock.DamageModifiers.Resistances.length > 0 && (
                    <>
                        <strong>Damage Resistances:</strong> {statBlock.DamageModifiers.Resistances.join(", ")}
                        <br />
                    </>
                )}
                {statBlock.DamageModifiers.Vulnerabilities.length > 0 && (
                    <>
                        <strong>Damage Vulnerabilities:</strong> {statBlock.DamageModifiers.Vulnerabilities.join(", ")}
                        <br />
                    </>
                )}
                ...
                {statBlock.Details.Senses.map(({ Name, Modifier }) => {
                    return (
                        <span key={Name}>
                            {Name} {Modifier} ft.{" "}
                        </span>
                ...
                <span className="justify-between">
                    ...
                    <span>
                        <strong>Proficiency Bonus:</strong> +{statBlock.ProficiencyBonus}
                    </span>
                </span>
            </Card>
            ...
            {statBlock.LegendaryActions && statBlock.LegendaryActions.Actions && statBlock.LegendaryActions.Actions.length > 0 && (
                <Card>
            ...
            {statBlock.MythicActions && statBlock.MythicActions.Actions && statBlock.MythicActions.Actions.length > 0 && (
                <Card>
